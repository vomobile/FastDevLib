 ...
 * {@link #isInvalidEvent(MotionEvent, int, int)}此处判断可滑动的范围
 */
public class SwipeBackLayout extends FrameLayout {
    ...
    private int deviceWidth;//屏幕宽度
    private int downPointX;//按下点的x位置
    ...
    private void init(Context context) {
        ...
        deviceWidth = DisplayUtil.getScreenWidth(context);
    ...
    /**
     * 根据第一次触摸的位置  判定此次滑动是否可以滑动关闭
     * @return
     */
    private boolean isCanMoveByX(){
        if(downPointX>deviceWidth/12){
            return true;
        }else {
            return false;
        }
